...
public class BanckingAccountService implements BanckingAccount {
	...
	public AccountTransactionDto getAccountTransactions(String accountId, String fromDate, String toDate) {
		...
		ApiBodyResponse<AccountTransactionDto> resposne = fabrickApiService.getAccountTransactions(accountId,
				Utils.validateDateInput(fromDate), Utils.validateDateInput(toDate));
		logger.debug("fabrickApiService.getAccountTransactions complete with status code {}", resposne.getStatus());
		return resposne.getPayload();
	}

	@Override
	public MoneyTransferResponseDto createMoneyTransfer(String accountId, MoneyTransferDto moneyTransferDto) {
		logger.debug("createMoneyTransfer: {}", accountId);
		ApiBodyResponse<MoneyTransferResponseDto> resposne = fabrickApiService.createMoneyTransfer(accountId, moneyTransferDto);
		logger.debug("fabrickApiService.createMoneyTransfer complete with status code {}", resposne.getStatus());
		return resposne.getPayload();
	}
}
